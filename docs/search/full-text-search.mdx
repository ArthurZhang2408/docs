...
<Warning>
**Node.js / TypeScript limitation:** Phrase queries cannot be performed across multiple columns simultaneously.

In the Node.js SDK, `PhraseQuery` accepts a single `column` (e.g. `new PhraseQuery("puppy runs", "text")`). If you need phrase matching across multiple fields, run separate phrase queries per column and merge results in your application, or use a different SDK that supports multi-column phrase queries.
</Warning>
