...
<Note>
Phrase queries are **single-column only** and cannot be used with `MultiMatchQuery`. If you need phrase matching across multiple columns, run separate `PhraseQuery` calls on each column and combine the results using a `BooleanQuery` with `Occur.Should` (OR logic):

```python
from lancedb.query import PhraseQuery, BooleanQuery, Occur

# Search for phrase in either "title" or "content" columns
query = BooleanQuery([
    (Occur.Should, PhraseQuery("machine learning", "title")),
    (Occur.Should, PhraseQuery("machine learning", "content")),
])
