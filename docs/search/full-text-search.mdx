...
<Note>
Phrase queries are **single-column**: they cannot be combined with multi-column full-text search (for example, a `MultiMatchQuery` across multiple columns). If you need phrase matching, run a `PhraseQuery` against one column at a time.
</Note>
