...
<Note>
**Phrase queries are single-column only.** You can use `PhraseQuery` against one column (e.g. `PhraseQuery("puppy runs", "text")`).
Searching across multiple columns is supported with `MultiMatchQuery`, but it only supports term/match queries (not phrases).

If you try to use a phrase query with multiple columns, the SDK will raise: `Phrase queries cannot be used with multiple columns.`
</Note>
