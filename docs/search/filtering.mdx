...
<Note>
In the TypeScript/Node.js SDK, use `.onlyIf(...)` to apply a SQL filter. (In Python, the equivalent method is `.where(...)`.)
</Note>
...
const filteredResult = await table
  .query()
  .onlyIf("(item IN ('foo', 'bar', 'baz')) AND (price > 15.0)")
  .limit(3)
  .toArray();
```
</CodeGroup>
...
const results = await table
  .search([100, 102])
  .onlyIf("(item IN ('foo', 'bar')) AND (price > 15.0)")
  .toArray();
```
</CodeGroup>
...
await table
  .search([100, 102])
  .onlyIf("(item IN ('foo', 'baz')) AND (price > 20.0)")
  .toArray();
```
</CodeGroup>
