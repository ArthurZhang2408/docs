...
<Info>
**SDK naming difference:** Python uses `.where(...)` for SQL filtering, while the Node.js/TypeScript SDK uses `.only_if(...)`.
</Info>
...
const filteredResult = await table
  .query()
  .only_if("(item IN ('foo', 'bar', 'baz')) AND (price > 15.0)")
  .limit(3)
  .toArray();
```
</CodeGroup>
...
const results = await table
  .search([100, 102])
  .only_if("(item IN ('foo', 'bar')) AND (price > 15.0)")
  .toArray();
```
</CodeGroup>
...
await table
  .search([100, 102])
  .only_if("(item IN ('foo', 'baz')) AND (price > 20.0)")
  .toArray();
```
</CodeGroup>
