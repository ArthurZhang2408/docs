...
LanceDB also integrates with [ConteXtualized Token Retriever (XTR)](https://arxiv.org/abs/2304.01982), an advanced retrieval model that prioritizes the most semantically important document tokens during search.

XTR is provided via the VoyageAI integration (model `voyage-context-3`). See [XTR with VoyageAI](/integrations/embedding/xtr) for setup, usage, and an example that combines XTR with multivector search.
