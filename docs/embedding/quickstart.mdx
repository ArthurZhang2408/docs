...
<Warning>
If you pass provider credentials (for example `api_key`) to `.create(...)`, LanceDB requires them to be provided as a **secure variable reference** like `api_key="$var:OPENAI_API_KEY"`. Hardcoding secrets directly (e.g. `api_key="sk-..."`) will raise an error.
</Warning>
