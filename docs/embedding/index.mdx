...
| `api_key` | `str` | The API key for the embedding provider. **Must be provided as a secure variable reference** (e.g. `api_key="$var:OPENAI_API_KEY"`) or via the provider's own environment variable support. Hardcoding secrets will raise an error. |
| `device` | `str` | The device to run the model on (e.g., "cpu", "cuda"). Defaults to automatic detection. |

<Warning>
Do not hardcode secrets (for example `api_key="sk-..."`) when creating embedding functions. LanceDB validates certain fields as *sensitive* and will raise an error unless the value is a secure variable reference of the form `"$var:ENV_VAR_NAME"`.
...
# Set in your shell, CI, or secrets manager
...
# LanceDB will read the value from the environment at runtime.
