...
import { RsCreateEmptyTableWithEmbedding } from '/snippets/embedding.mdx';
...
<Note>
In the Rust SDK, you can attach embedding definitions when creating an empty table using the builder returned by `create_empty_table(...)`.
</Note>

<CodeGroup>
  <CodeBlock filename="Rust" language="Rust" icon="rust">
    {RsCreateEmptyTableWithEmbedding}
  </CodeBlock>
</CodeGroup>
