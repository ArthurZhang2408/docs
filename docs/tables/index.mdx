...
<Warning>
`table.to_arrow()` (and `table.to_pandas()`) are only supported for **local** tables.
When connected to **LanceDB Cloud / remote tables**, these methods raise `NotImplementedError`.

For remote tables, use `table.search(...).to_arrow()` / `table.search(...).to_pandas()` to
materialize query results instead of exporting the entire table.
</Warning>
