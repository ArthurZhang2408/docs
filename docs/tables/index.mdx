...
Remote tables support the core operations (ingest, search, update/delete), but some local-only
convenience methods are not available.

In particular, in the Python SDK `RemoteTable.to_arrow()` and `RemoteTable.to_pandas()` are not
implemented.
</Info>
...
<Note>
This workflow applies to local (embedded) tables. In the Python SDK, remote tables (Cloud/Enterprise)
do not currently support `to_arrow()` / `to_pandas()`.
</Note>
