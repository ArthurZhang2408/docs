...
If you want LanceDB to store data directly in your own cloud object storage, you can also connect using an object storage URI:

- `s3://<bucket>/<path>` (Amazon S3 and S3-compatible stores)
- `gs://<bucket>/<path>` (Google Cloud Storage)
- `az://<container>/<path>` (Azure Blob Storage)

See [Storage options](/storage/) for examples including `storage_options` / `storageOptions`.
