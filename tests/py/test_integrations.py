...
def test_reranking_mrr_usage() -> None:
    ...
    # Hybrid search: combine vector + FTS rankings
    tbl.create_fts_index("text", replace=True)
    result = (
        tbl.search("hello", query_type="hybrid")
        .rerank(reranker=reranker)
        .to_list()
    )

    # Multivector search: combine multiple vector result sets
    rs1 = tbl.search("hello", vector_column_name="vector").limit(10).with_row_id(True)
    rs2 = tbl.search("goodbye", vector_column_name="vector").limit(10).with_row_id(True)
